class Chat_Peer
{
  constructor(id = null, settings, vueapp)
  {
    ...
    this.chatapp = new Vue({
      ...
      data: {
        ...
        startPeer: function(event){ that.startPeer() },
        leaveQueue: function(event){ that.leaveQueue() },
        submitMsg: function(event){ that.submitMsg() }
      }
    });
  ...
  incomingDataHandler(data, c)
  {
    ...
    {
      var message = json_data["data"]
      this.chatapp.chatlog.push(message)
    ...
    if(json_data["type"] == "queue")
    {
      ...
      {
        // Note: can make it drop all occurances with a while loop

        // leave the queue once
  ...
  submitMsg(){
    console.log("Sending msg: " + this.chatapp.msg)

    var message = {
      "user": this.me.id,
      "body": this.chatapp.msg
    }
